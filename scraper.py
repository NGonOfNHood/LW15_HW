...
def fetch_url():
    url = _url.get()
    config['images'] = []
    _images.set(())  # Initialize as an empty tuple
    try:
...
if __name__ == "__main__":
    ...
    _fetch_btn = ttk.Button(_url_frame, text='Fetch Images', command=fetch_url)
    ...
    _fetch_links_btn.grid(row=0, column=3, sticky=W, padx=5)
    ...
    _scrape_btn = ttk.Button(_mainframe, text='Save', command=save)
